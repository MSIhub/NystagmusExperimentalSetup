...
def main():
    ...
    fileName = f"sync_info_{(int(time.time() * 1000))}.info"
    try:
        f = open(fileName, 'w')
        # 3. Measure clock offset once
        offset = measure_clock_offset(socket, clock_function=local_clock)
        f.write(f"Clock offset (1 measurement): {offset} seconds\n")
        # 4. Measure clock offset more reliably to account for network
        # latency variance (multiple measurements)
        ...
        f.write(f"Mean clock offset ({number_of_measurements} measurements): "
        ...
        f.write(
            f"Pupil time (calculated locally): {pupil_time_calculated_locally}\n")
        # 6. Log information to a file
    finally:
        f.close()
...
def measure_clock_offset_stable(socket, clock_function, nsamples=10):
    ...
    offsets = [measure_clock_offset(socket, clock_function)
